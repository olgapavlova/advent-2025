...
int main(void) {
  ...
  while((read = getline(&line, &len, g)) != -1) {
    ...
    // обнуляем вектор текущей проверки
    for(int i = 0; i < Q; i++) { check[i] = 0; }
...
#if 0
      // показываем состояние цепочки
      printf("Цепочка: ");
      for(int i = 0; i < Q; i++) {
        if(conn[i] == 1) printf("%d ", i);
      }
      printf("\n");
#endif 
    }
    ...
    for(int i = 0; i < Q; i++) {
      sum += conn[i];
      // if(conn[i]) printf("%d ", i);
    }
    // printf("\n");

    // показываем длину цепи после установления связи
    // printf("a[%d]: %ld\t%ld\t%ld\n", a, p[a].x, p[a].y, p[a].z);
    // printf("b[%d]: %ld\t%ld\t%ld\n", b, p[b].x, p[b].y, p[b].z);
    // printf("sum: %d\n", sum);
